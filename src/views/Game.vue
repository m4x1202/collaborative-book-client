      ...
      <v-row no-gutter justify="start" class="mt-10">
        ...
        <v-col>
          <v-card color="yellow darken-1">
            <v-row>
              <v-col> Room: {{ roomId }} </v-col>
              <v-col> Stage: {{ currentStage }}/{{ lastStage }} </v-col>
            </v-row>
          </v-card>
          <v-card class="mt-n1">
            <v-card-text>
              <v-form id="submit-story-form" @submit.prevent="submitStory">
                <v-textarea
                  ...
                  outlined
                  no-resize
                  placeholder="The last book line will appear here once created"
                  :value="output"
                ></v-textarea>
                <v-textarea
                  :disabled="!(userStatus == 'writing')"
                  no-resize
                  clearable
                  autofocus
                  v-model="input"
